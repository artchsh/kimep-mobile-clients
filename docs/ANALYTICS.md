...
| **Domain** | There is no real domain — this is a native app. Put the same value the app reports as its `hostname` so the dashboard lines up: **`kz.kimep.mobile.unofficial`**. For API-only tracking the domain is not used for validation; Umami mainly uses it to restrict where the browser script runs. |
...
for the apps (events arrive tagged with `hostname = kz.kimep.mobile.unofficial`).
...
```json
{
  ...
  "payload": {
    "website": "<website id>",
    "hostname": "kz.kimep.mobile.unofficial",
